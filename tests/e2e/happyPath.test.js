...
describe('Happy path', () => {
  ...
  test('welcome screen is shown on open', async () => {
    const page = await openPopup(browser, { bookmarks: BOOKMARKS, fetchResponses: FETCH_RESPONSES });
  ...
  test('scan completes → results screen with correct stats and table', async () => {
    ...
    const checkedCount = await page.$$eval('#dead-list input[type="checkbox"]:checked', cbs => cbs.length);
