...
describe('Edge cases', () => {
  ...
  test('no bookmarks → "No bookmarks to process", no table, no delete button', async () => {
    ...
    const tableHidden = await page.$eval('#results-table-container', (el) =>
  ...
  test('cancel during scan → returns to welcome screen', async () => {
    ...
    const slowFetchResponses = Object.fromEntries(children.map((c) => [c.url, { status: 200 }]));
    ...
    await page.evaluate(() => {
      globalThis.fetch = () =>
        new Promise((resolve) => setTimeout(() => resolve({ ok: true, status: 200 }), 400));
    });
        ...
        children: [
          {
            ...
            children: [
              {
                id: '10',
                title: '<img src=x onerror="window._xss=1">',
